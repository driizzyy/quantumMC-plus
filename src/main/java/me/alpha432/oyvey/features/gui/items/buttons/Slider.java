...
public class Slider
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        ...
        // Draw background
        RenderUtil.rect(context, this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, 0x22555555);
        
        // Draw progress bar
        float progress = this.partialMultiplier();
        int fillColor = !this.isHovering(mouseX, mouseY) ? 
        ...
        RenderUtil.rect(context, this.x, this.y, (this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * progress, this.y + (float) this.height - 0.5f, fillColor);
